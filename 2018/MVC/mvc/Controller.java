...
public class Controller extends MouseInputAdapter implements MouseWheelListener
{
	...
	{
		Point aPoint = new Point(0, aMouseWheelEvent.getWheelRotation());// 通常の場合y座標移動
		Integer someModifiers = aMouseWheelEvent.getModifiersEx();
		Boolean controlDown = (someModifiers & InputEvent.CTRL_DOWN_MASK) != 0;
		if (controlDown)
		{
			aPoint = new Point(aMouseWheelEvent.getWheelRotation(), 0);
		} // コントロールキーが押下された場合x座標移動
		this.scrollBy(aPoint, aMouseWheelEvent);
		return;
	}
